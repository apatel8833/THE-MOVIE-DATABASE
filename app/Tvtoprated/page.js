...
import { TvtopratedData } from '../Redux/TvtopratedSlice';
import Link from 'next/link';
import Loader from '../Components/Loader';

const page = () => {
    const dispatch = useDispatch();
    const Movies = useSelector((state) => state.tvtoprated.tvtoprated)
    console.log("tvtoprated", Movies);
    const[load,setLoad] = useState(1)


    useEffect(() => {
        dispatch(TvtopratedData(load)); 
            ...
            <div className='w-full h-100 p-4'>
                <div>
                    <div className='flex content-between items-end w-96'>
                        <h1 className='fs-3 fw-medium'>On TV..</h1>
                    </div>
